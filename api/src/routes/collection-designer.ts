...
${sample.map((r) => JSON.stringify(r)).join('\n').slice(0, 16000)}${evidenceBlock}`
  ...
  app.post('/apply', async (req, reply) => {
    const b = req.body as {
      ...
      import_processor?: 'service' | 'procedure'
    }
    ...
    if (b.create_import) {
      for (const c of plan.collections) {
        ...
        const processor = b.import_processor === 'procedure' ? 'procedure' : 'service'
        const procName = `import_${c.collection}`.slice(0, 120)
        const stagingTable = `staging_${c.collection}`.slice(0, 120)
        try {
          let defRow: Record<string, unknown>
          let m2mSkipped: string[]
          if (processor === 'service') {
            // Items-service processor (default): every write goes through
            // createOne/updateOne — revisions, activity, rules, hooks — and
            // an unchanged re-import writes nothing.
            const svc = generateServiceConfig(c)
            m2mSkipped = svc.skipped
            defRow = {
              key: c.collection,
              label: c.display_name || c.collection,
              description: `Generated by the Collection Designer — items-service import into ${c.collection}${c.import_key?.length ? ` keyed on ${c.import_key.join(', ')}` : ' (append-only)'}`,
              staging_table: stagingTable,
              procedure: null,
              loader: null,
              is_active: 1,
              processor: 'service',
              service_config: JSON.stringify(svc.config),
            ...
            const gen = generateImportProc(c, procName, stagingTable)
            m2mSkipped = gen.skipped
            if (!(await db.schema.hasTable(stagingTable))) {
              await db.schema.createTable(stagingTable, (t) => {
                t.increments()
                for (const col of gen.stagingColumns) t.text(col)
              })
            }
            await db.raw(gen.sql)
            defRow = {
              key: c.collection,
              label: c.display_name || c.collection,
              ...
              processor: null,
              service_config: null,
              staging_columns: JSON.stringify(gen.stagingColumns),
              procedure_body: gen.sql,
          ...
          importPipeline.push({
            collection: c.collection,
            procedure: processor === 'service' ? `${c.collection} (items service)` : procName,
            staging_table: stagingTable
          })
        } catch (err) {
          errors.push(
...
/** Map a designer plan collection onto a service-mode import config —
 *  columns keyed by the staged-import worker's normalized headers, designer
 *  types folded into the processor's coercions, match_field relations into
 *  lookups (with on_missing carried through). */
export function generateServiceConfig(c: DesignCollection): {
  ...
  for (const f of mapped) {
    const col = normalizeHeader(f.source_column!)
    stagingColumns.push(col)
    if (f.relation?.match_field) {
      ...
      const type =
        f.relation || f.type === 'integer' || f.type === 'bigInteger'
          ? 'int'
          : f.type === 'decimal' || f.type === 'float'
            ? 'number'
            : f.type === 'boolean'
              ? 'boolean'
              : f.type === 'date'
                ...
                : f.type === 'datetime'
                  ? 'datetime'
                  : 'string'
      columns[col] = { field: f.field, ...(type === 'string' ? {} : { type }) }
    }
  }
  return {
    config: {
      collection: c.collection,
      match_by: c.import_key ?? [],
      columns
    },
    stagingColumns,
    skipped
