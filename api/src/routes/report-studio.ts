...
function sanitizeAlertFilters(raw: unknown): string | null {
...
export async function reportStudioRoutes(app: FastifyInstance) {
  ...
  app.post('/widget-presets', { preHandler: requireAuth }, async (req, reply) => {
    ...
    if (!b.name || !b.widget_type)
      return reply.code(400).send({ error: 'name and widget_type required' })
    const now = new Date()
  ...
  app.patch('/widget-presets/:presetId', { preHandler: requireAuth }, async (req, reply) => {
    ...
    void logActivity({
      ...
      user: req.user?.id ?? null,
      comment: `preset ${id}: ${Object.keys(patch)
  ...
  app.get<{ Params: { id: string } }>('/:id', { preHandler: requireAuth }, async (req, reply) => {
    ...
    return reply.send({
      data: { ...formatReport(report, widgets), editable: canEditReport(report, req) }
    })
  })
  ...
  app.put<{
    ...
    if (!canReadReport(report, req)) return reply.code(403).send({ error: 'Forbidden' })
    if (req.body == null) {
      await db('nivaro_report_subscriptions').where({ report: report.id, user: req.user!.id }).del()
      await logActivity({
    ...
    async (req, reply) => {
      ...
      for (const col of collections) {
        try {
          ...
          const rows = (await db(col).distinct(field).whereNotNull(field).limit(100)) as Array<
      ...
      const fieldDefs = rawFields
        .map((f: unknown) =>
          typeof f === 'string'
            ? { field: f, label: f }
            : {
                field: String((f as { field?: unknown })?.field ?? ''),
...
Entity filter fields MUST be among: ${fieldList.join(', ') || '(none available — return empty entity_filters)'}. Match the user's words against the user-facing labels (e.g. "Zone 1" belongs to the field labeled "Zone"). Only set date_range when the prose mentions a TIME period — a funding year is a filter field, not a date range. Today is ${new Date().toISOString().slice(0, 10)}.`
      ...
      try {
        ...
        // AI calls are logged (ai-generate/ai-summarize precedent, and the
        // sibling report-ai-build).
        void logActivity({
          action: 'report-ai-filters',
          user: req.user?.id ?? null,
          collection: 'nivaro_report_defs',
  ...
  app.delete<{ Params: { id: string; snapId: string } }>(
    ...
    async (req, reply) => {
      ...
      void logActivity({
        ...
        collection: 'nivaro_report_defs',
        item: String(report.id),
        comment: `snapshot ${req.params.snapId}`
      })
      return reply.send({ data: { deleted: true } })
    }
  )
  ...
  app.post<{
    ...
    const id =
      typeof inserted === 'object' && inserted !== null
        ...
        : (inserted as number)
    void logActivity({
      action: 'report-annotation-create',
      user: req.user?.id ?? null,
      collection: 'nivaro_report_defs',
      item: String(report.id),
      comment: note.slice(0, 120)
    })
  ...
  app.delete<{ Params: { id: string; annId: string } }>(
    ...
    async (req, reply) => {
      ...
      void logActivity({
        action: 'report-annotation-delete',
        user: req.user?.id ?? null,
        collection: 'nivaro_report_defs',
        item: String(report.id),
        comment: String(row.note ?? '').slice(0, 120)
      })
      return reply.send({ data: { deleted: true } })
    }
  )
  ...
  app.post<{ Params: { id: string }; Body: { name?: string; description?: string } }>(
    ...
    async (req, reply) => {
      ...
      const name =
        String(req.body?.name ?? '')
          .trim()
          .slice(0, 120) || `${report.name} template`
  ...
  app.delete<{ Params: { templateId: string } }>(
    ...
    async (req, reply) => {
      ...
      void logActivity({
        ...
        user: req.user?.id ?? null,
        comment: `template "${String(tpl.name ?? tpl.id).slice(0, 80)}"`
      })
      return reply.send({ data: { deleted: true } })
    }
  )
  ...
  app.post<{ Params: { id: string; widgetId: string }; Body: { target_report_id?: string } }>(
    ...
    async (req, reply) => {
      ...
      void logActivity({
        action: 'report-widget-copy',
        user: req.user?.id ?? null,
        collection: 'nivaro_report_defs',
        item: String(target.id),
        comment: `widget ${req.params.widgetId} copied from report ${report.id}`
      })
