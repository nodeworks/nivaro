...
let centroidCache: {
  at: number
  rows: Array<{ id: number; label: string; lat: number; lng: number }>
} | null = null

async function regionCentroids(): Promise<
...
export async function commandCenterRoutes(app: FastifyInstance) {
    ...
    const [flow, risk, people, throughput, health, activity] = await Promise.all([
      ...
      safe(
          ...
          for (const r of scopeRows) {
            ...
            try {
              ...
              if (Array.isArray(vals))
                regionByUser.set(String(r.user).toUpperCase(), vals.map(Number))
            } catch {
              /* corrupt scope row */
            }
          }
        ...
        },
        {
          ...
          idle_count: 0,
          names: [] as string[],
          by_region: [] as Array<{ region_id: number; count: number }>,
          offices: [] as Array<{
            lat: number
            lng: number
            label: string
  ...
  app.get<{ Querystring: { collection?: string } }>('/geo', async (req, reply) => {
    ...
    const lngField = names.has('longitude')
      ? 'longitude'
      : names.has('lng')
        ? 'lng'
        : names.has('lon')
    ...
    const rows = ((result as { data?: Array<Record<string, unknown>> }).data ?? []).filter(
      (r) =>
        Number(r[latField]) !== 0 &&
        Number.isFinite(Number(r[latField])) &&
        Number.isFinite(Number(r[lngField]))
    )
    ...
    try {
      if (ids.length > 0) {
        ...
        for (const i of inst)
          stateById.set(String(i.item), { key: i.key, label: i.label, color: i.color })
      }
    } catch {
