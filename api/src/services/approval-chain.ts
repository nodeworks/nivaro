...
export async function buildApprovalChain(instanceId: string): Promise<ChainStep[]> {
  ...
  const visited = new Set<string>(history.map((h) => h.to_state))
  const currentSort = states.find((s) => s.id === instance.current_state)?.sort ?? 0
  ...
  const steps = states.filter(
    (s) =>
      ...
      (s.id === instance.current_state ||
        visited.has(s.id) ||
        (s.sort > currentSort && !isEscape(s.key)))
  )
  const upcoming = steps.filter((s) => s.sort > currentSort && s.id !== instance.current_state)
  const next = upcoming[0]
  const ownerRequests: OwnerResolutionRequest[] = [instance.current_state, next?.id]
  ...
  return steps.map((s) => {
    const status: ChainStep['status'] =
      s.id === instance.current_state ? 'current' : s.sort > currentSort ? 'upcoming' : 'done'
