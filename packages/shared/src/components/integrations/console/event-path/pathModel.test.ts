...
describe('formatOffset', () => {
  ...
  it('switches to hours and days for long chains', () => {
    expect(formatOffset(3_599_000)).toBe('+59m 59s')
    expect(formatOffset(3_600_000)).toBe('+1h 0m')
    expect(formatOffset(73_500_000)).toBe('+20h 25m')
    expect(formatOffset(171_893_947)).toBe('+2d 0h')
    expect(formatOffset(86_400_000 * 3 + 7_200_000)).toBe('+3d 2h')
    expect(formatOffset(-7_200_000)).toBe('\u22122h 0m')
  })
  it('prints a true minus sign for steps before the root', () => {
    expect(formatOffset(-500)).toBe('\u2212500 ms')
    expect(formatOffset(-1200)).toBe('\u22121.2 s')
    expect(formatOffset(-125_000)).toBe('\u22122m 5s')
    expect(formatOffset(-0.2)).toBe('+0 ms')
    expect(formatOffset(-500)).not.toContain('+')
  })
})
