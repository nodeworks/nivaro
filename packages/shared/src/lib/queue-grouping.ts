// Minimal shape queue-grouping needs — inlined here instead of importing the
// admin-only QueueItemRow (admin/src/pages/QueueDetail.tsx) which would pull
// a page component into @nivaro/shared. Generic so callers passing the full
// QueueItemRow (with additional fields) get those fields back untouched on
// QueueGroup.rows.
export interface QueueGroupingRow {
  collection: string
  state: string | null
...
export interface QueueGroup<T extends QueueGroupingRow = QueueGroupingRow> {
  key: string
  rows: T[]
  breached: number
  atRisk: number
...
export function deriveGroupKey<T extends QueueGroupingRow>(row: T, attribute: string): string {
...
export function buildGroups<T extends QueueGroupingRow>(
  rows: T[],
  attribute: string
): QueueGroup<T>[] {
  const map = new Map<string, QueueGroup<T>>()
