...
export function NivaroForm({
  ...
  const renderOneField = (field: FormFieldDescriptor): React.ReactNode => {
    ...
    if (renderField)
      return <React.Fragment key={field.field}>{renderField(field, defaultRender)}</React.Fragment>
  ...
  const autoBody = () => {
    ...
    return (
      <>
        ...
        {form.visibleGroups.map((g) => renderGroupBlock(g, form.fieldsByGroup.get(g.key) ?? []))}
      </>
    )
  }
