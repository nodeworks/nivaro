  ...
  content: [
    ...
    { type: 'h3', text: 'Transition requirements' },
    {
      ...
      items: [
        ...
        'requirements (optional): data-entry gates enforced at execute time (422 + a fill-in dialog). Type child_fields requires listed fields on every related child row (scalar inputs, m2o pickers for FK columns, m2m multi-selects writing junction rows); type record_fields collects fields on the transitioning record itself (text inputs, m2o pickers) — e.g. confirm an order number at submit time.',
        'optional_when (child_fields): waive a required field per row when a controlling field (a column or an m2m alias) holds one of a list of values — {"<field>": {"field": "<controlling>", "in": [..], "placeholder": "Auto-assigned"}}. Instead of typing ids, "in_query": {"collection", "filter": {col: value | [values] | null}, "value_field"} reads the list from a business collection at evaluation time (500 rows max) and can be combined with a static "in". A query that is malformed, fails, or matches nothing drops the rule — the field stays required, never accidentally waived.',
        'review_when (child_fields): {"field": "<record field>", "in": [..]} (or a list — any match) on the transitioning record brings the dialog back with EVERY row, filled or not, when e.g. the last partner push failed ("push_status": ["error"]) — so a line can be corrected before trying again; "review_message" sets the banner text. The dialog\'s own re-submit sends reviewed: true, which skips review_when (incomplete rows still block), so it asks once per attempt.',
