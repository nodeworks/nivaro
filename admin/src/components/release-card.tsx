...
export function ReleaseCard() {
  ...
  const start = useMutation({
    ...
    onError: (e: { response?: { status?: number; data?: { error?: string } } }) =>
      toast.error(
        e.response?.status === 409
          ? 'A release is already running'
          : (e.response?.data?.error ?? 'Could not start')
      )
  })
  ...
  return (
    ...
    >
      ...
      {confirm && plan && (
        ...
        >
          <p>{outcomeSentence(plan, bump)}</p>
          <div className='mt-2 flex gap-2'>
            <Button
              size='sm'
              onClick={() => start.mutate({ bump })}
      ...
      {openId && (
        <RunView key={openId} id={openId} onResume={(from) => start.mutate({ bump, from })} />
      )}
      ...
      {status.data.runs.length > 0 && (
        <details className='mt-3 text-[12px]'>
          ...
          <ul className='mt-1 space-y-0.5' data-release-history>
...
function RunView({ id, onResume }: { id: string; onResume: (from: Stage) => void }) {
  const qc = useQueryClient()
  ...
  const tone: Record<string, string> = {
    ...
    skipped: 'border-dashed border-slate-300 text-slate-400'
  }
  return (
    <div className='mt-3' data-release-run={id} data-release-run-state={run?.state}>
      <div className='flex flex-wrap gap-1.5'>
        ...
        {run?.state === 'running' && (
          <Button size='sm' variant='outline' onClick={() => cancel.mutate()} data-release-cancel>
            Cancel
          </Button>
        )}
        {run?.state === 'failed' && run.failed_stage && (
          <Button size='sm' onClick={() => onResume(run.failed_stage!)} data-release-resume>
      ...
      <pre
        className='mt-2 max-h-72 overflow-auto rounded-md bg-slate-950 p-3 font-mono text-[11px] leading-snug text-slate-100'
        data-release-log
      >
        {log || '…'}
      </pre>
