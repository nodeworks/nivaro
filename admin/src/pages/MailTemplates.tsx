...
import { MailHarness } from '@/components/mail-harness'
...
export default function MailTemplates() {
  ...
  const seedFromTemplate = () => {
    ...
    for (const m of body.matchAll(
      /\{%-?\s*(?:for|if|unless|assign)\s+[^%]*?\bin\s+([a-zA-Z_][a-zA-Z0-9_]*)/g
    ))
      vars.add(m[1])
                ...
                <div className='flex shrink-0 flex-wrap items-center gap-2 border-b border-slate-200 bg-white px-4 py-2.5 dark:border-border dark:bg-card'>
                  ...
                  {detail?.overridden && (
                    <button
                      ...
                      onClick={() => {
                        if (
                          window.confirm('Revert to the file default? The override is deleted.')
                        ) {
                          revert.mutate()
                        }
                      }}
