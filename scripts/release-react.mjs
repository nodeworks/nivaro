...
const pkgPath = 'packages/react/package.json';
...
if (bump === 'major') newVersion = `${major + 1}.0.0`;
  ...
  console.error('Usage: pnpm react:release patch|minor|major|<version>');
  console.error(`Current version: ${pkg.version}`);
  process.exit(1);
}

const tag = `@react-${newVersion}`;
...
run(['add', 'packages/react/package.json']);
run(['commit', '-m', `chore: release @nivaro/react@${newVersion}`]);
run(['tag', tag]);
...
console.log(`\n✓ @nivaro/react@${newVersion} tagged as ${tag} and pushed`);
