...
/** sh, but concurrent — for the waits and checks that can overlap. Same
 *  output rules, same StageError on a non-zero exit. */
function shAsync(cmd, args, { cwd = ROOT, quiet = false, allowFail = false } = {}) {
  return new Promise((done) => {
    ...
    child.on('error', (err) => done({ status: 1, stdout, stderr: `${stderr}${err.message}` }))
...
function detectChanges() {
  ...
  // A package is "changed" since ITS last release tag, not since the app's:
  // the chain's own `chore: release @nivaro/react@x` commit lands after the
  // v-tag, so measured from there every release re-published react (and
  // re-pinned every frontend) for a version-only bump. The bump commit
  // itself never counts.
  const since = (pattern) => {
    const tag = sh('git', ['describe', '--tags', '--abbrev=0', '--match', pattern], { quiet: true, allowFail: true }).stdout
    return git(['diff', '--name-only', `${tag || last}..HEAD`]).split('\n').filter(Boolean)
  }
  // Neither the bump commit nor the sdk README (regenerated by sync-docs on
  // EVERY app release) is a change worth a publish.
  const changedUnder = (list, re, ignore) => list.some((f) => re.test(f) && !ignore.includes(f))
  return {
    last,
 ...
 *  background while a later stage is the current one. */
async function waitForWorkflow(stage, repo, workflow, accept) {
  await sleep(25_000) // the run does not exist the instant the tag lands
  const id = sh('gh', ['run', 'list', '-R', repo, '--workflow', workflow, '-L1', '--json', 'databaseId', '-q', '.[0].databaseId'], { quiet: true }).stdout
  log(`watching ${workflow} run ${id}`)
  emit(stage, 'progress', `watching ${workflow} run ${id}`)
  const res = await shAsync('gh', ['run', 'watch', '-R', repo, id, '--exit-status'], { quiet: true, allowFail: true })
  if (res.ok) return
...
async function until(what, fn, { tries = 30, every = 20_000, stage = null } = {}) {
  for (let i = 1; i <= tries; i++) {
    if (await fn()) return
    log(`${what} — not yet (${i}/${tries})`)
    emit(stage ?? currentStage, 'progress', `${what} — not yet (${i}/${tries})`)
    await sleep(every)
  }
...
async function main() {
  ...
  try {
    if (runs('preflight')) {
      ...
      await Promise.all(
        dirs.map((dir) =>
          shAsync('npx', ['tsc', '--noEmit'], { cwd: resolve(ROOT, dir), quiet: true }).catch((err) => {
            throw new StageError(`typecheck ${dir}: ${err.message}`)
          })
        )
      )
      emit('preflight', 'ok')
    ...
    // The frontends need the npm packages, not the image; only the deployment
    // push needs the image. So the frontends pin and push while the image is
    // still building (it is the slow artifact, ~5 min). On a resume that
    // re-enters `artifacts`, a frontend already pinned + committed is pushed
    // as it is, never re-bumped.
    let frontendsDone = false
    const runFrontends = async () => {
      frontendsDone = true
      if (!(p.wantReact || p.wantSdk)) {
        emit('frontends', 'skip', 'nothing to pin')
        return
      }
      ...
      for (const f of cfg.frontends) {
        ...
        const lockNow = readFileSync(resolve(cwd, 'pnpm-lock.yaml'), 'utf8')
        const pinned = Object.entries(pins).every(([name, v]) => pkg.dependencies?.[name] === v && lockNow.includes(`${name}@${v}`))
        if (pinned && !git(['status', '--short', 'package.json', 'pnpm-lock.yaml'], { cwd })) {
          log(`${f.name}: already pins ${Object.values(pins).join(' + ')} — pushing what is there`)
    ...
    if (runs('artifacts')) {
      ...
      const imageWait = (async () => {
        if (repo) await waitForWorkflow('artifacts', repo, 'docker-hub.yml', async () => (cfg.image ? imageExists(cfg.image, V) : false))
        // The gate. A green workflow is a claim; the registry is the artifact.
    ...
    if (runs('frontends') && !frontendsDone) await runFrontends()
    else if (!runs('frontends')) emit('frontends', 'skip', `resumed from ${FROM}`)
    ...
    // Exit outright: a background wait (the image watch runs while the
    // frontends pin) would otherwise keep a failed chain alive for minutes.
    process.exit(1)
  }
}
